...
import { TypingAnimation } from "@/components/codeforge/ui/typing-animation";
...
export function PlanSearchCard({ rootRef, status, showDialog, showTyping, popoverPosition = "top" }: PlanSearchCardProps) {
    return (
        ...
        >
            ...
            {showTyping ? (
                <TypingAnimation words={["Why did we move auth to database sessions?", "Which command fixed the broken import?", "What changed before the last working build?"]} loop className="text-sm font-medium" />
            ) : (
                <p className="text-sm font-medium">Ask memory</p>
            )}
